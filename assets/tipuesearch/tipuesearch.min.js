(function($) {
	$.fn.tipuesearch = function(options) {
		...
		return this.each(function() {
			...
			$('#tipue_search_content').hide().html('<div class="tipue_search_spinner"><div class="tipue_search_rect1"></div><div class="tipue_search_rect2"></div><div class="rect3"></div></div>');
			if(set.mode == 'live') {
			...
			function getTipueSearch(start, replace) {
				...
				d = $.trim(d);
				window.history.replaceState(null, null, "?q=" + d);
					...
					if(standard) {
						...
						for(var i = 0; i < tipuesearch_in.pages.length; i++) {
							...
							for(var f = 0; f < d_w.length; f++) {
								...
								if(tipuesearch_in.pages[i].author.search(pat) != -1) {
									var m_c = tipuesearch_in.pages[i].author.match(pat).length;
									score += (10 * m_c);
								}
								if(tipuesearch_in.pages[i].external.search(pat) != -1) {
									var m_c = tipuesearch_in.pages[i].external.match(pat).length;
									score += (10 * m_c);
								}
								if(tipuesearch_in.pages[i].keywords.search(pat) != -1) {
									var m_c = tipuesearch_in.pages[i].keywords.match(pat).length;
									score += (10 * m_c);
								}
